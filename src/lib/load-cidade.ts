...
let indexCache: CidadeIndexEntry[] | null = null;

export function loadCidadesIndex(): CidadeIndexEntry[] {
  // Milhares de páginas leem o índice no build; parse uma vez só.
  indexCache ??= JSON.parse(fs.readFileSync(path.join(GENERATED, 'cidades-index.json'), 'utf-8'));
  return indexCache!;
}
